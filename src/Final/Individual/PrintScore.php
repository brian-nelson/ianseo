 ...
 *********************************/
	...
	foreach(AvailableApis() as $Api) {
		if(empty($_SESSION[$Api.'-Mode'])) continue;
...
//			AND EvTeamEvent=0 AND EvFinalFirstPhase!=0
//		group by
...
//			if($dif)
//			{
//				echo '<br/>'. get_text('FinalShort', 'Tournament').' ' . get_text('EventDetails', 'Tournament', array($r->EvFinEnds, $r->EvFinArrows, $r->EvFinSO)) ;
//			}
...
//Scores per singolo match
	...
	// recupera per questo torneo quanti formati ci sono...
//	$query="(select distinct EvElimEnds, EvElimArrows, EvElimSO, EvTeamEvent, EvMixedTeam from Events where EvTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND EvTeamEvent=0 AND EvFinalFirstPhase!=0)"
//		. "UNION (select distinct EvFinEnds, EvFinArrows, EvFinSO, EvTeamEvent, EvMixedTeam from Events where EvTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND EvTeamEvent=0 AND EvFinalFirstPhase!=0)"
//		. " ORDER BY EvTeamEvent, EvMixedTeam, EvElimEnds, EvElimArrows, EvElimSO";
...
//		echo '<td><a href="PDFScoreMatch.php?Blank=1&Rows=' . $r->EvElimEnds . '&Cols='.$r->EvElimArrows.'&SO='.$r->EvElimSO.'" class="Link" target="PrintOut">';
//		echo '<img src="../../Common/Images/pdf.gif" alt="' . get_text('Score1Page1Match') . '" border="0"><br>';
//		echo get_text('Score1Page1Match');
//		echo '<br/>'. get_text('EventDetails', 'Tournament', array($r->EvElimEnds, $r->EvElimArrows, $r->EvElimSO)) ;
//		echo '</a></td>';
	}
...
//Eventi
	...
	echo get_text('Event') . '<br><select name="Event[]" multiple="multiple" id="p_Event" onChange="javascript:ChangeEvent(0,\'p\');" size="10">';
	foreach($Events as $Event => $EventName) {
		echo '<option value="' . $Event . '">' . $Event . ' - ' . get_text($EventName,'','',true)  . '</option>';
	}
	echo '</select>';
	...
	echo '<input name="BigNames" type="checkbox" />' . get_text('BigNames','Tournament') ;
