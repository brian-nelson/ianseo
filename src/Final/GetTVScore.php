...
//		debug_svela($myRow);
		...
		if($EnCours) {
			...
			$Out.='<tr style="height:20%"><td id="name1">'.$myRow->name1.'</td><td id="name2">'.$myRow->name2.'</td></tr>';;
			$Out.='<tr style="width:100%;height:80%"><td id="Score1">'.$myRow->score1.'</td><td id="Score2">'.$myRow->score2.'</td></tr>';
			$OutMainString*=0.75;
		}
	}
	$Out.='</table>';
	...
	$Out.='<style>
	#Score1, #Score2, #MiniScore1, #MiniScore2, .ScoreArrows1, .ScoreArrows2 {font-size:'.($WinWidth/($OutMainString*$Scale)).'px;text-align:center;}
...
//	$JS_SCRIPT[]='td {border:1px solid black;}';
	...
	$JS_SCRIPT[]='<script type="text/javascript" src="'.$CFG->ROOT_DIR.'Final/Fun_AJAX.js.php"></script>';
