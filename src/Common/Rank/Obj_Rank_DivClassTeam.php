	 ...
	 */
		protected function safeFilter()
		{
			...
			if (!empty($this->opts['enid'])) {
				$filter.=" AND CONCAT(TeCoId,'|',TeSubTeam,'|',TeEvent) IN (SELECT CONCAT(TcCoId,'|',TcSubTeam,'|',TcEvent) FROM TeamComponent WHERE TcId=" . intval($this->opts['enid']). " AND TcFinEvent=0) " ;
			}
