...
define ("ProgramVersion","2014.08.10");	// Versione del programma "San Lorenzo"
...
function set_qual_session_flags() {
	...
	$q=safe_r_sql("select EvCode, EvTeamEvent, EvFinalFirstPhase, EvShootOff, EvE1ShootOff, EvE2ShootOff, EvElim1, EvElim2 from Events where EvTournament={$_SESSION['TourId']} AND EvFinalFirstPhase!=0");
 ...
 */
function Set_Tournament_Option($key, $value, $unset=false) {
	if(empty($_SESSION['TourId']) || $_SESSION['TourId']<=0) return;
	$ConstToStore=array();
	$q=safe_r_sql("select ToOptions from Tournament where ToId={$_SESSION['TourId']}");
	$r=safe_fetch($q);
	if(!empty($r->ToOptions)) $ConstToStore=unserialize($r->ToOptions);
	if($unset) {
...
function PrintCrackError($popup=false, $errore='CrackError', $Module='Common') {
	global $CFG;
	include('Common/Templates/head'.($popup?'-popup':'').'.php');
	echo get_text($errore, $Module);
 ...
 */
	$q="SELECT IF(DATE_SUB(NOW(), INTERVAL {$days} DAY)>ParValue,1,0) AS MustCheck FROM Parameters WHERE ParId='ChkUp' ";
	//print $q;exit;
	$r=safe_r_sql($q);
...
function GetWebDirectory($file) {
	global $CFG;
	return $CFG->ROOT_DIR.dirname(str_replace($CFG->DOCUMENT_PATH, '', $file));
}
...
function AvailableApis() {
	global $CFG;
	if(ProgramRelease=='FITARCO') return false;
	$ret=array();
