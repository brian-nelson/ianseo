...
@font-face {
    ...
    src: url("<?php echo $CFG->ROOT_DIR; ?>Common/pdf/fonts/londo___.ttf");
}
...
function deleteRow() {
	var Rows=document.getElementById('scrolltab0').rows;
	if(Rows.length > 0) {
		for(var n=0; n<Rows[0].cells.length; n++) {
			Rows[0].cells[n].style.overflow='hidden';
			Rows[0].cells[n].style.height='0px';
		}
		Rows[0].style.overflow='hidden';
		Rows[0].style.height='0px';
	}
}

</script>
<script type="text/javascript" src="<?php print $CFG->ROOT_DIR.'TV/TVscript.js'; ?>"></script>
</head><body onload="deleteRow()" id="body">
<div id="Content">
