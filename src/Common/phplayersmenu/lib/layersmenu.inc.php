	...
	)
{
	...
	for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {	// this counter scans all nodes of the new menu
		...
		if ($this->tree[$cnt]['child_of_root_node']) {
			...
			if ($this->tree[$cnt]['not_a_leaf']) {
				$this->tree[$cnt]['onmouseover'] = (!empty($_SESSION['ClickMenu']) ? ' onclick' : ' onmouseover').'="moveLayerX1(' . "'" . $this->tree[$cnt]['layer_label'] . "', '" . $menu_name . "') ; LMPopUp('" . $this->tree[$cnt]['layer_label'] . "'" . ', false);"';
			} else {
				$this->tree[$cnt]['onmouseover'] = (!empty($_SESSION['ClickMenu']) ? ' onclick' : ' onmouseover').'="shutdown();"';
			}
			$t->setVar(array(
			...
			if (!empty($this->_hasIcons[$this->tree[$cnt]['father_node']])) {
				$t_sub->setVar(array(
					'iconsrc'	=> $this->tree[$cnt]['iconsrc'],
					'iconwidth'	=> $this->tree[$cnt]['iconwidth'],
					'iconheight'	=> $this->tree[$cnt]['iconheight'],
	...
	)
{
	...
	for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {	// this counter scans all nodes of the new menu
		...
		if ($this->tree[$cnt]['child_of_root_node']) {
			...
			if ($this->tree[$cnt]['not_a_leaf']) {
				$this->tree[$cnt]['onmouseover'] = (!empty($_SESSION['ClickMenu']) ? ' onclick' : ' onmouseover').'="moveLayerX(' . "'" . $this->tree[$cnt]['layer_label'] . "') ; moveLayerY('" . $this->tree[$cnt]['layer_label'] . "') ; LMPopUp('" . $this->tree[$cnt]['layer_label'] . "'" . ', false);"';
			} else {
				$this->tree[$cnt]['onmouseover'] = (!empty($_SESSION['ClickMenu']) ? ' onclick' : ' onmouseover').'="shutdown();"';
			}
			$t->setVar(array(
