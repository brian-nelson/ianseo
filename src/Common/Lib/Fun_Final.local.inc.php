		...
		if ($TeamEvent==0)		//individuali
		{
			$query= "
				SELECT
					...
					IF(e1.EnFirstName IS NOT NULL, CONCAT(e1.EnFirstName,' ',e1.EnName),'') AS name1,
					IF(c1.CoCode IS NOT NULL, c1.CoCode,'') AS countryCode1,
					IF(c1.CoName IS NOT NULL, c1.CoName,'') AS countryName1,
					IF(e2.EnFirstName IS NOT NULL, CONCAT(e2.EnFirstName,' ',e2.EnName),'') AS name2,
		...
		else		// team
		{
			...
			$query.= "
				FROM
					...
					LEFT JOIN
						(SELECT TfcCoId, TfcSubTeam, TfcEvent, GROUP_CONCAT(CONCAT(EnName,' ', EnFirstName) SEPARATOR ', ') as Name1
					...
					LEFT JOIN
						(SELECT TfcCoId, TfcSubTeam, TfcEvent, GROUP_CONCAT(CONCAT(EnName,' ', EnFirstName) SEPARATOR ', ') as Name2
						FROM TeamFinComponent
						INNER JOIN Entries ON TfcId=EnId
						WHERE TfcTournament=" . StrSafe_DB($_SESSION['TourId']) . "
