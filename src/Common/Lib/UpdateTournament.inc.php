...
//		$Gara['Tournament']['ToDouble']='0';
	...
	if($Gara['Tournament']['ToDbVersion']<'2019-04-10 21:29:02') {
		// procedura di aggiornamento della gara alla versione attuale del database
		$ToKeep=array();
		if(array_key_exists('OdfScheduleStatus', $Gara)) {
			$Gara['OdfMessageStatus']=array();
            foreach ($Gara['OdfScheduleStatus'] as $key => $row) {
            	foreach($row as $fld=>$val) {
	                $Gara['OdfMessageStatus'][$key][str_replace('OdfSchSt','Oms', $fld)]=$val;
	            }
	            $Gara['OdfMessageStatus'][$key]['OmsType']='SCHED';
            }
        }
	}
