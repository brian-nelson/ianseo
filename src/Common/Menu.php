...
?>
<link rel="shortcut icon" href="<?php print $Path_WWW; ?>LOGOS/shortcut_icon_phplm.png">

<?php // javascript per ingrandire logo on over ?>
<script type="text/javascript" src="<?php echo $CFG->ROOT_DIR ?>Common/Fun_ResizeImg.inc.js"></script>

<?php // javascript per popup eccetera ?>
<script type="text/javascript" src="<?php echo $CFG->ROOT_DIR ?>Common/Fun_JS.inc.js"></script>
...
//if(!$_SESSION['WINHEIGHT']) {
	...
	echo 'document.write(\'<LINK href="http://'.$_SERVER['SERVER_NAME'].$CFG->ROOT_DIR.'Common/Load_css.php?video=\'+video+\'&videowidth=\'+videowidth+\'" rel="stylesheet" type="text/css">\');';
	//if($ERROR_REPORT) echo 'alert(video);';
	echo '</script>';
//}

	if($_SESSION['WINHEIGHT']) echo "<style>#Content {height:".max(180,$_SESSION['WINHEIGHT']-120)."px}</style>";
...
// 	 	if($Title!='COMP') debug_svela($List);
