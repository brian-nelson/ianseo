 ...
 */
	function CreateElimRows($event,$elim)
	{
		$max=0;
		$ret=true;
	...
	// numero di righe da creare
		$q="
			...
			FROM
				Events
			WHERE
				EvCode='{$event}' AND EvTournament={$_SESSION['TourId']} AND EvTeamEvent=0
		";
		$r=safe_r_sql($q);
	//print $q.'<br><br>';exit;
		if ($r && safe_num_rows($r)==1)
		{
			...
			if ($max>0)
			{
				...
				// id,girone,evento,torneo,rank di qualifica,score,hits,gold,x,rank,timestamp
					$tuple[]="(0,{$e},'{$event}',{$_SESSION['TourId']},{$i},0,0,0,0,0,'0000-00-00 00:00:00')";
				}
						...
						if ($type==0 || $type==3)
						{
							if ($team==0)
							{
								$q
									...
									. "WHERE "
										. "IndTournament={$_SESSION['TourId']} AND IndEvent='{$event}' "
								;
							}
							elseif ($team==1)
							{
								$q
									...
									. "SET "
										...
										. "TeTimeStampFinal=0 "
									. "WHERE "
										. "TeTournament={$_SESSION['TourId']} AND TeEvent='{$event}' AND TeFinEvent=1 "
								;
							}
						}
						elseif ($type==1)
