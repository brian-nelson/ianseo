...
if (isset($PdfData->Data['Items']) && count($PdfData->Data['Items'])>0)
{
	foreach($PdfData->Data['Items'] as $Group => $Rows) {
		if(!$pdf->SamePage(20)) $pdf->AddPage();

	   	$pdf->SetFont($pdf->FontStd,'B',10);
		$pdf->Cell(190, 6,  $Rows[0]->FirstLetter, 1, 1, 'C', 1);
		...
		foreach($Rows as $MyRow) {
			...
			$pdf->Cell(11, 4 * $secondaryTeam,  $MyRow->IsAthlete ? (!empty($PdfData->BisTarget) && (intval(substr($MyRow->TargetNo,1)) > $PdfData->NumEnd) ? str_pad((substr($MyRow->TargetNo,0,-1)-$PdfData->NumEnd),3,"0",STR_PAD_LEFT) . substr($MyRow->TargetNo,-1,1) . ' bis'  : $MyRow->TargetNo) : '', 1, 0, 'R', 0);
			if(!$PdfData->HideCols && !$TargetFace)
			{
