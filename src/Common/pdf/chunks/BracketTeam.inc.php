	...
	foreach($section['phases'] as $Phase => $Items) {
		...
		foreach($Items['items'] as $Match) {
			...
			if($Match['tiebreakDecoded']) {
				$pdf->SetFont($pdf->FontStd,'',6);
				$pdf->Cell($MisTie, $CellHeight, $Match['tiebreakDecoded'], 0, 0, 'L', 0);		//*
			} elseif($Match['tie']==1) {
				$pdf->Cell($MisTie, $CellHeight, "*", 0, 0, 'C', 0);		//*
			} else {
				$pdf->Cell($MisTie, $CellHeight, "", 0, 0, 'C', 0);		//No Tie
			}
			$pdf->SetXY($MyX, $OrgY+$CellHeight);
			if($Match['oppTiebreakDecoded']) {
				$pdf->SetFont($pdf->FontStd,'',6);
				$pdf->Cell($MisTie, $CellHeight, $Match['oppTiebreakDecoded'], 0, 0, 'L', 0);		//*
