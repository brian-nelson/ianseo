...
foreach($PdfData->Data['Items'] as $EvCode => $MyRows) {
	...
	if($check->EvElimType==3 or $check->EvElimType==4) {
	...
	if($check->EvElimType==4) {
		...
		foreach($MyRows as $MyRow) {
			...
			if($targetNo != substr($MyRow->TargetNo,0,-1)) {
				...
				if($MyRow->EvElimType==3 or $MyRow->EvElimType==4) {
					$arc[]=$MyRow->Score.(isset($MyRow->TiebreakDecoded) ? ' T.'.$MyRow->TiebreakDecoded : '');
				}
				$arc[]=$MyRow->Ranking."#";
				$arc[]=$MyRow->DOB;
		...
		foreach($MyRows as $MyRow) {
			...
			if((!is_null($MyRow->EventCode) && $OldEvent != $MyRow->EventCode) || (is_null($MyRow->EventCode) && $OldEvent && '#@#@#')) {
				...
				if(!$_SESSION['MenuElimPoolDo'] and !is_null($MyRow->SesName)) {
			...
			if($OldMatchPhase!=$MyRow->GrPhase or ($MyRow->EvElimType==3 and $OldTitle!=$PdfData->MatchTitles[intval($MyRow->FinMatchNo/2)*2])) {
				if($MyRow->EvElimType==3) {
					...
					$PrintSubTitle=false;
					// after 2 groups start a new page
					if($OldMatchPhase<$MyRow->GrPhase) {
						$Group++;
						if($Group==3 and !$PoolPage) {
							$pdf->addPage();
					...
					if($PrintSubTitle) {
						...
						$pdf->SetFont('', 'b', $OldFont+3);
						$pdf->SetY($pdf->lastY+3.5);
						$pdf->Cell(0, 5, get_text('PoolName', 'Tournament', chr(64+$Group)), '',1);
						$pdf->lastY+=7;
						$pdf->SetFont('', '', $OldFont);
					}

					if($MyRow->FinMatchNo==12 or $MyRow->FinMatchNo==10) {
						$OldFont=$pdf->getFontSizePt();
						$pdf->SetFont('', 'b', $OldFont+3);
						$pdf->SetY($pdf->lastY+3.5);
						$pdf->Cell(0, 5, get_text('PoolName', 'Tournament', $Group==2 ? 'AB' : 'CD'), '',1);
						$pdf->lastY+=7;
						$pdf->SetFont('', '', $OldFont);
					}
					// print the title of the match
					$pdf->lastY+=3.5;
					$pdf->SetY($pdf->lastY);
					$pdf->cell(0, 0, $PdfData->MatchTitlesWA[intval($MyRow->FinMatchNo/2)*2]);
