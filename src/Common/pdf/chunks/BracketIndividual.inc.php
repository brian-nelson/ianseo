...
// 			if($DrawMatch) debug_svela($Match);
			...
			if($ShowTargetNo && ($Match['target'] or $Match['oppTarget'])) {
				if($FirstPhase) {
					...
					$pdf->SetFont($pdf->FontStd,'I',6);
// 					debug_svela($Match);
					$pdf->Cell($MisName + $AddSize + $MisScore + 2*$AddSize + ($PrintCountry ? $MisCountry : 0), 2.5, "T# " . ltrim($Match['target'],'0') . ($ShowSchedule && $Match['scheduledTime']!='00:00' && $Match['scheduledDate']!='00-00-0000' ?  ($DateShown ? '' : ' ' . $Match['scheduledDate']) . " @ "  . $Match['scheduledTime'] : '') , 0, 0, 'L', 0);
					if($Match['target']!=$Match['oppTarget']) {
			...
			if($ShowSchedule && $Match['scheduledTime']!='00:00' && $Match['scheduledDate']!='00-00-0000') {
			...
			if($DrawMatch or $Match['saved']) {
				$pdf->Cell($MisName + ($PrintCountry ? 2 :1 ) * $AddSize, $Cella,  $FirstPhase ? $Match['athlete'] : $Match['familyName'] . ' ' . ($Match['givenName'] ? substr($Match['givenName'],0,1) . '.' : '') , ($Match['position']!=0 || $Match['oppTie']==2 ? 1 : 0), 0, 'L', 0);
			} else {
				$pdf->SetFont($pdf->FontStd,'I',5.5);
			...
			if($DrawMatch or $Match['oppSaved']) {
				$pdf->Cell($MisName + ($PrintCountry ? 2 :1 ) * $AddSize, $Cella,  $FirstPhase ? $Match['oppAthlete'] : $Match['oppFamilyName'] . ' ' . ($Match['oppGivenName'] ? substr($Match['oppGivenName'],0,1) . '.' : '') , ($Match['oppPosition']!=0 || $Match['tie']==2 ? 1 : 0), 0, 'L', 0);
			} else {
				$pdf->SetFont($pdf->FontStd,'I',5.5);
				$pdf->Cell($MisName + $MisTie + $MisScore, $Cella,  $rankData['meta']['saved'] , 0, 0, 'L', 0);
				$pdf->SetFont('','');
			...
			if($Match['tiebreakDecoded']) {
				$pdf->SetFont($pdf->FontStd,'',6);
				$pdf->Cell($MisTie, $Cella, $Match['tiebreakDecoded'], 0, 0, 'L', 0);
			} elseif($Match['tie']==1) {
				$pdf->Cell($MisTie, $Cella, "*", 0, 0, 'C', 0);		//*
			} else {
				$pdf->Cell($MisTie, $Cella, "", 0, 0, 'C', 0);		//No Tie
			}
			$pdf->setXY($MyX, $OrgY+$Cella);
			if($Match['oppTiebreakDecoded']) {
				$pdf->SetFont($pdf->FontStd,'',6);
				$pdf->Cell($MisTie, $Cella, $Match['oppTiebreakDecoded'], 0, 0, 'L', 0);
