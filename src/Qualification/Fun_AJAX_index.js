			...
			if (!document.getElementById('chk_BlockAutoSave').checked)
			{
				if (confirm(MsgAreYouSure))
				{
					if (XMLHttp.readyState==XHS_COMPLETE || XMLHttp.readyState==XHS_UNINIT)
					{
						...
						XMLHttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
						XMLHttp.onreadystatechange=Went2Home_StateChange;
						XMLHttp.send(null);
					}
				}
			}
...
function Went2Home_StateChange()
{
	// se lo stato � Complete vado avanti
	if (XMLHttp.readyState==XHS_COMPLETE)
	{
...
// Intercetto gli errori di Firefox
	...
	if (Error==0)
	{
		...
		{
			SetStyle('Row_'+Id,'');
			document.getElementById('TD_' + Id).innerHTML="<?= get_text('Went2Home', 'Tournament') ?>";
		}
		else
		{
			SetStyle('Row_'+Id,'NoShoot');
			document.getElementById('TD_' + Id).innerHTML="<?= get_text('BackFromHome', 'Tournament') ?>";
		}
			
		for (var i=1; i <= 8; ++i)
		{
			if (document.getElementById('d_QuD' + i + 'Score_' + Id))
			{
				document.getElementById('d_QuD' + i + 'Score_' + Id).value='0';
				document.getElementById('d_QuD' + i + 'Score_' + Id).disabled=(Retired==1);
			}
			if (document.getElementById('d_QuD' + i + 'Gold_' + Id))
			{
				document.getElementById('d_QuD' + i + 'Gold_' + Id).value='0';
				document.getElementById('d_QuD' + i + 'Gold_' + Id).disabled=(Retired==1);
			}
			if (document.getElementById('d_QuD' + i + 'Xnine_' + Id))
			{
				document.getElementById('d_QuD' + i + 'Xnine_' + Id).value='0';
				document.getElementById('d_QuD' + i + 'Xnine_' + Id).disabled=(Retired==1);
			}
		}
