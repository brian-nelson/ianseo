			...
			if (count($errors)==0)
			{
				$query
					= "UPDATE "
						...
						. "ON EnId=QuId "
					. "SET "
						. "Qualifications.QuSession=" . StrSafe_DB($endSession) . " "
						. ", QuBacknoPrinted=0, Qualifications.QuTargetNo=CONCAT(" . StrSafe_DB($endSession) . ",RIGHT(CONCAT('000',SUBSTRING(QuTargetNo,2,".TargetNoPadding.")+(" . (intval($destFrom)-intval($sourceFrom)) . ")), ".TargetNoPadding."),RIGHT(QuTargetNo,1)) ";

//				if ($sourceFrom!='' && $destFrom!='')
//					$query .= ", Qualifications.QuTargetNo=CONCAT(" . StrSafe_DB($endSession) . ",RIGHT(CONCAT('000',SUBSTRING(QuTargetNo,2,".TargetNoPadding.")+(" . (intval($destFrom)-intval($sourceFrom)) . ")), ".TargetNoPadding."),RIGHT(QuTargetNo,1)) ";
...
//				if ($sourceFrom!=0)
//					$query .= " AND QuTargetNo>=CONCAT(" . StrSafe_DB($startSession) . ", RIGHT(CONCAT('000'," . StrSafe_DB($sourceFrom) . "),LENGTH(QuTargetNo)-2)," . (is_numeric(substr($sourceFrom,-1)) ? "'A'" : '') . ") ";
//				if ($sourceTo!=0)
//					$query .= " AND QuTargetNo<=CONCAT(" . StrSafe_DB($startSession) . ", RIGHT(CONCAT('000'," . StrSafe_DB($sourceTo) . "),LENGTH(QuTargetNo)-2)," . (is_numeric(substr($sourceTo,-1)) ? "'Z'" : '') . ") ";
				//print $query;exit;
				$rs=safe_w_SQL($query);
