...
/* salvo */
	if (isset($_REQUEST['Command']))
	{
		 ...
		 */
			...
			$Insert
				= "INSERT INTO Entries (EnId,EnIocCode,EnTournament,EnDivision,EnClass,EnAthlete,EnSubClass,EnAgeClass,EnCountry,EnSubTeam,EnCountry2,EnCountry3,EnDob,EnCode,EnName,EnFirstName,EnSex,EnTargetFace,EnStatus,EnIndClEvent,EnTeamClEvent,EnIndFEvent,EnTeamFEvent,EnTeamMixEvent,EnWChair,EnDoubleSpace) "
				. "VALUES("
				. ($id!=0 ? StrSafe_DB($id) : "''") . ","
				. StrSafe_DB($_REQUEST['LupSelect']) . ","
				. StrSafe_DB($_SESSION['TourId']) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnDivision_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnClass_'])) . ","
				. StrSafe_DB($EnAthlete) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnSubClass_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnAgeClass_'])) . ","
				. StrSafe_DB($EnCountries['']) . ","
				. StrSafe_DB($_REQUEST['d_e_EnSubTeam_']) . ","
				. StrSafe_DB($EnCountries['2']) . ","
				. StrSafe_DB($EnCountries['3']) . ","
				. StrSafe_DB(trim($EnDob)) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnCode_'])) . ","
				. StrSafe_DB($EnName) . ","
				. StrSafe_DB($EnFirstName) . ","
				. StrSafe_DB($_REQUEST['d_e_EnSex_']) . ","
				. StrSafe_DB($_REQUEST['d_e_EnTargetFace_']) . ","
				. StrSafe_DB($_REQUEST['d_e_EnStatus_']) . ", "
				. StrSafe_DB(trim($_REQUEST['d_e_EnIndClEvent_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnTeamClEvent_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnIndFEvent_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnTeamFEvent_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnTeamMixEvent_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnWChair_'])) . ","
				. StrSafe_DB(trim($_REQUEST['d_e_EnDoubleSpace_'])) . " "
				. ") ON DUPLICATE KEY UPDATE "
				. "EnIocCode=" . StrSafe_DB(trim($_REQUEST['LupSelect'])) . ","
				. "EnDivision=" . StrSafe_DB(trim($_REQUEST['d_e_EnDivision_'])) . ","
				...
				. "EnCountry=" . StrSafe_DB($EnCountries['']) . ","
				. "EnSubTeam=" . StrSafe_DB($_REQUEST['d_e_EnSubTeam_']) . ","
				. "EnCountry2=" . StrSafe_DB($EnCountries['2']) . ","
				. "EnCountry3=" . StrSafe_DB($EnCountries['3']) . ","
				. "EnDob=" . StrSafe_DB($EnDob) . ","
				. "EnCode=" . StrSafe_DB(trim($_REQUEST['d_e_EnCode_'])) . ","
				...
				. "EnIndClEvent=" . StrSafe_DB($_REQUEST['d_e_EnIndClEvent_']) . ","
				. "EnTeamClEvent=" . StrSafe_DB($_REQUEST['d_e_EnTeamClEvent_']) . ","
				. "EnIndFEvent=" . StrSafe_DB($_REQUEST['d_e_EnIndFEvent_']) . ","
				. "EnTeamFEvent=" . StrSafe_DB($_REQUEST['d_e_EnTeamFEvent_']) . ","
				. "EnTeamMixEvent=" . StrSafe_DB($_REQUEST['d_e_EnTeamMixEvent_']) . ","
				. "EnWChair=" . StrSafe_DB($_REQUEST['d_e_EnWChair_']) . ","
				. "EnDoubleSpace=" . StrSafe_DB($_REQUEST['d_e_EnDoubleSpace_']) . " "
			;
			...
			$Update
				...
				. "QuSession=" . StrSafe_DB($_REQUEST['d_q_QuSession_']) . ", "
				. "QuTargetNo=" . StrSafe_DB(($_REQUEST['d_q_QuSession_']!=0 ?$_REQUEST['d_q_QuSession_'] :'' ) . $_REQUEST['d_q_QuTargetNo_']) . " "
...
//	exit;
	...
	$t=safe_r_sql("select LookUpPaths.*, ToId is not null as LupDefault from LookUpPaths left join Tournament on LupIocCode=ToIocCode and ToId={$_SESSION['TourId']}");
	while($u=safe_fetch($t))
	{
		if($u->LupIocCode!='FITA' or file_exists($CFG->DOCUMENT_PATH.'/Modules/IanseoTeam/LookupFitaId.php'))
		{
			$selected='';
			if($id)
			{
				if($record['ioccode']==$u->LupIocCode)
				{
			...
			$comboLup.= '<option value="' . $u->LupIocCode . '"'.$selected.'>'.$u->LupIocCode.'</option>';
		}
	}
	$comboLup.='</select>';
...
<form name="Frm" method=post action="<?php print $_SERVER['PHP_SELF'];?>">
	...
	<table class="Tabella" id="idAthList">
		...
		<tr>
			...
			<td>
				<input type="text" maxlength="9" name="d_e_EnCode_" id="d_e_EnCode_" value="" onblur="SetCompleteFlag();" onfocus="SetCompleteFlag();" onkeyup="CercaMatr(true);"/>
				<?php /* ?>
				&nbsp;
		...
		<tr>
			...
			<td colspan="3"><input type="text" maxlength="30" size="50" name="d_c_CoName3_" id="d_c_CoName3_" value=""/></td>
		</tr>
