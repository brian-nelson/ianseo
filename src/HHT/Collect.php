						...
						if (count($Results)!=0)
						{
							...
							//Verifico il tipo di dato che torna
								...
								$Sql = "INSERT INTO HhtData (HdEnId, HdTournament, HdTargetNo, HdRealTargetNo, HdLetter, HdDistance, HdFinScheduling, HdTeamEvent, HdArrowStart, HdArrowEnd, HdArrowString, HdHhtId, HdTimeStamp) "
									. "VALUES ($EnId, " . StrSafe_DB($_SESSION['TourId']) . ", '" . $TargetNo. "', '" . substr($TargetNo, 0, -1). "', '" . substr($TargetNo,-1). "', '" . $Dist . "', '" . $scheduling . "', '" .  $team ."', '" . $v["FirstArr"] . "', '" . $v["LastArr"] . "', '" . $ArrowString . "', " . StrSafe_DB($_REQUEST['x_Hht']) . ", now()) "
									. "ON DUPLICATE KEY UPDATE HdTimeStamp=now(), HdEnId=$EnId, HdArrowEnd='" . $v["LastArr"] . "', HdArrowString='" . $ArrowString . "', HdHhtId=" . StrSafe_DB($_REQUEST['x_Hht']);
								safe_w_sql($Sql);
