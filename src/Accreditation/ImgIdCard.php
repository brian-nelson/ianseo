...
function draw_pip($r) {
	...
	switch($r->IceType) {
		...
		case 'Picture':
			if(!isset($im)) $im=$CFG->DOCUMENT_PATH."Common/Images/Photo.jpg";
		case 'Image':
			if(!isset($im)) {
		...
		case 'CompDetails':
			if(!isset($Text)) $Text=$_SESSION['TourWhere'].' - '.TournamentDate2StringShort($_SESSION['TourWhenFrom'], $_SESSION['TourWhenTo']);
		case 'Athlete':
			if(!isset($Text)) {
				switch($r->IceContent) {
		...
		case 'Category':
			if(!isset($Text)) $Text=get_text('Category', 'BackNumbers');
		case 'Session':
...
// 				debug_svela($pos);
	...
	// Calculate the dimensions of the box containing the text
	$font=dirname(dirname(__FILE__))."/Common/pdf/fonts/"
