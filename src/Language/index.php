<?php
	...
	if(!empty($_REQUEST['UpdateLanguage']) and preg_match('/^[A-Z-]+$/', $_REQUEST['UpdateLanguage'])) {
		...
		if( $package=@file_get_contents("http://translations.ianseo.net/getpackage.php?lang={$_REQUEST['UpdateLanguage']}")) {
	...
	if(!empty($_REQUEST['FindLanguage'])) {
		if( $package=@file_get_contents("http://translations.ianseo.net/getlanguages.php")) {
