...
function CreateStandardDivisions($TourId, $Type=1, $SubRule=0) {
	...
	if(!in_array($Type, array(1,2,3,4,18))) CreateDivision($TourId, $i++, 'AN', 'Arco Nudo');
	if(in_array($Type, array(1,2,4))) CreateDivision($TourId, $i++, 'C1', 'Compound W1');
	if($Type>=9 and $Type<=13) CreateDivision($TourId, $i++, 'LB', 'Long Bow');
	if($Type<9 or $Type>13) CreateDivision($TourId, $i++, 'VI', 'Visually Impaired');
	if(in_array($Type, array(9,10,12))) CreateDivision($TourId, $i++, 'AI', 'Arco Istintivo');
}

function CreateStandardClasses($TourId, $SubRule, $Field='', $Type=0) {
	$i=1;
	$Fita=in_array($Type, array(1,2,4,18));
	switch($SubRule) {
		case '1':
			if($Field=='3D') {
				...
				if($Fita) {
					CreateClass($TourId, $i++,  1, 100, 0, 'M', 'M', 'Maschile', 1, 'C1,VI');
					CreateClass($TourId, $i++,  1, 100, 1, 'F', 'F', 'Femminile', 1, 'C1,VI');
				}
				if($Field=='FIELD') {
...
function CreateStandardStudClasses($TourId, $SubRule, $Field='') {
	$i=1;
