...
if(!$AllInOne) {
	die('La compétition est du mauvais type!');
}
...
if(count($EventCodes) != 0) {
    $rank = Obj_RankFactory::create('AbsTeam', array('events' => $EventCodes));
    ...
    if (isset($_REQUEST['R']) AND !IsBlocked(BIT_BLOCK_TEAM)) {
        ...
        if (count($Grids2Handle) != 0) {
            ...
            // WE USE THE STANDARD TEAM CREATION!!!
        ...
        foreach ($EventCodes as $Event) {
            ...
            if ($r = safe_fetch($q) and !$r->EvShootOff) {
	            // inserts the rank of this event in Teams so to have all done as a fake qualification
	            $QualEvent=substr($Event, 1);

	            // gets the ranking status of the original event
	            $rank = Obj_RankFactory::create('FinalTeam', array('tournament' => $_SESSION['TourId'], 'eventsR'=>$QualEvent));
	            $rank->read();
	            $data=$rank->getData();

	            $q=safe_r_sql("select Teams.*, CoCode 
					from Teams
					inner join Countries on CoId=TeCoId and CoTournament=TeTournament
					where TeTournament={$_SESSION['TourId']} and TeEvent=".StrSafe_DB($QualEvent));
	            while($r=safe_fetch($q)) {
		            $item=$data['sections'][$r->TeEvent]['items'][$r->CoCode.'_'.$r->TeSubTeam];
		            $r->TeScore=$item['mainPoints'];
		            $r->TeGold=$item['winPoints']-$item['loosePoints'];
		            $r->TeXnine=$item['winPoints'];
		            $r->TeRank=$item['rank'];
		            $r->TeEvent='F'.$r->TeEvent;
		            unset($r->CoCode);
		            $SQL2=array();
		            foreach($r as $k=>$v) {
			            $SQL2[]="$k=".StrSafe_DB($v);
		            ...
		            $Insert=implode(',', $SQL2);
		            safe_w_sql("insert into Teams set $Insert on duplicate key update $Insert");
	            }
	            // inserts the TeamComponents on the final event getting the same ones as the original event...
	            $q=safe_r_sql("delete from TeamComponent where TcTournament={$_SESSION['TourId']} and TcEvent=".StrSafe_DB($Event));
	            $q=safe_r_sql("select * from TeamComponent where TcTournament={$_SESSION['TourId']} and TcEvent=".StrSafe_DB($QualEvent));
	            while($r=safe_fetch($q)) {
		            $r->TcEvent='F'.$r->TcEvent;
		            $SQL2=array();
		            foreach($r as $k=>$v) {
			            $SQL2[]="$k=".StrSafe_DB($v);
		            }
		            safe_w_sql("insert ignore into TeamComponent set ".implode(',', $SQL2));
	            }

	            // recalculates the rank of the final event
    ...
    $Sql = "SELECT EvCode, EvEventName, EvNumQualified, EvShootOff, GROUP_CONCAT(DISTINCT itemNo) as SoCt 
        ...
        "WHERE EvTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND EvTeamEvent=1 AND EvFinalFirstPhase=2 AND EvCodeParent='' " .
