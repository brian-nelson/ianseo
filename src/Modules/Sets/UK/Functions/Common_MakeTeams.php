...
{ 
	...
	for($level=$startLevel; $level<=$endLevel; $level++)
	{
		$Select = "
			...
			AND EnTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND CONCAT(IF(EnDivision='B','R',EnDivision), EnClass) IN (" . implode(",",$clMen) . "))
		UNION ALL
			...
			WHERE EnAthlete=1 AND EnCountry<>0  AND EnTeamClEvent=1 AND EnStatus<=1 AND QuScore<>0 AND CoCode<>'0'
			" . (!is_null($Societa) ? "AND EnCountry=" . StrSafe_DB($Societa) : "") . " 
			AND EnTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND CONCAT(IF(EnDivision='B','R',EnDivision), EnClass) IN (" . implode(",",$clWomen) . "))
		UNION ALL
			...
			WHERE EnAthlete=1 AND EnCountry<>0  AND EnTeamClEvent=1 AND EnStatus<=1 AND QuScore<>0 AND CoCode<>'0'
			" . (!is_null($Societa) ? "AND EnCountry=" . StrSafe_DB($Societa) : "") . " 
			AND EnTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND CONCAT(IF(EnDivision='B','R',EnDivision), EnClass) IN (" . implode(",",array_merge($clMen,$clWomen)) . "))
