...
$q=safe_r_sql("select min(length(trim(QuD5Arrowstring) )) MinEnds, max(length(trim(QuD5Arrowstring) )) MaxEnds  from Qualifications inner join Entries on QuId=EnId and EnTournament=$TourId and QuSession=8");
$r=safe_fetch($q);

$StartEnds=intval($r->MinEnds/3)*3;
if($r->MinEnds==$r->MaxEnds) $StartEnds-=3;
if($StartEnds<0) $StartEnds=0;
