<?php
	...
	if (empty($_SESSION['TourId']) && (!CheckTourSession() || is_null($schedule) || is_null($team)))
		exit;
			...
			// controllo la request
				...
				if(!empty($target))
				{
					$xml.='<m>'
						...
						.'<r>' . $r . '</r>'
						.'<ev>' . $myRow->event. '</ev>'
						.'<ph>' . get_text(namePhase($myRow->firstPhase,$myRow->phase). '_Phase') . '</ph>'
						.'<evn><![CDATA[' . $myRow->eventName. ']]></evn>'
						.'<t>' . $target . '</t>'
						.'<n1><![CDATA[' . $myRow->name1 . ']]></n1>'
						.'<cn1><![CDATA[' . $myRow->countryName1 . ']]></cn1>'
						.'<ar1>' . strlen(str_replace(' ','',$myRow->arrowString1)). '</ar1>'
						.'<sar1>' . strlen(str_replace(' ','',$myRow->tiebreak1)). '</sar1>'
						.'<n2><![CDATA[' . $myRow->name2 . ']]></n2>'
						.'<cn2><![CDATA[' . $myRow->countryName2 . ']]></cn2>'
						.'<ar2>' . strlen(str_replace(' ','',$myRow->arrowString2)). '</ar2>'
						.'<sar2>' . strlen(str_replace(' ','',$myRow->tiebreak2)). '</sar2>'
